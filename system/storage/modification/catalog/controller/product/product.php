		...
		if ($product_info) {
			...
			if ($this->config->get('config_stock_display') && !empty($product_features_options)) {
				...
				foreach ($query->rows as $query_quantity) {
					...
					if (!isset($warehouse)) {
						$warehouse = $query_quantity['quantity'];
					}

					$query = $this->db->query("SELECT `u`.`name`, `u`.`rus_name1`, `pu`.`ratio`, `pu`.`product_feature_id` FROM `" . DB_PREFIX . "product_unit` `pu` LEFT JOIN `" . DB_PREFIX . "unit` `u` ON (`pu`.`unit_id` = `u`.`unit_id`) WHERE `pu`.`product_id` = " . $product_id);
					if ($query->num_rows) {
						$product_units[$unit_id] = array(
							'rus_name'				=> $query->row['rus_name1'],
							'name'					=> $query->row['name'],
							'ratio'					=> $query->row['ratio'],
