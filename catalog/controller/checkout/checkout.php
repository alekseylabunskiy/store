...
class ControllerCheckoutCheckout extends Controller { 
	...
	{ //echo "<pre>"; print_r($this->session->data); echo "</pre>";
		...
		$data['payment'] = $this->load->controller('extension/payment/');
  				...
  				if ($method) {
  					if($cart_has_recurring > 0){
  						if (method_exists($this->{'model_extension_payment_' . $result['code']},'recurringPayments')) {
  							if($this->{'model_extension_payment__' . $result['code']}->recurringPayments() == true){
...
//				$data['shipping_method'] = $this->request->post['shipping_method'];				
	...
	// Display prices
			...
			$files = glob(DIR_APPLICATION . 'extension/controller/total/*.php');
