<?php
class ControllerExtensionModuleThemeFeatured extends Controller {
	public function index($setting) {
		$this->load->language('extension/module/theme_featured');
        ...
        //$this->document->addScript('catalog/view/theme/'. $this->config->get('config_theme') .'/js/carousel_ini.js');
        ...
        $data['title_about'] = $this->language->get('title_about');
        ...
        if (isset($setting['category_name'])) {
            $data['category_name'] = $setting['category_name'];
        }
				...
				if ($product_info) {
                    ...
                    foreach ($additional_images as $additional_image) {
                        $data['additional_images'][] = array(
                            'image' => $this->model_tool_image->resize($additional_image['image'], $setting['width'], $setting['height']),
                            'image2x' => $this->model_tool_image->resize($additional_image['image'], $setting['width']*2, $setting['height']*2)
                        );
                    }
		...
		if ($data['products']) {
			return $this->load->view('extension/module/theme_featured', $data);
		}
	}
}